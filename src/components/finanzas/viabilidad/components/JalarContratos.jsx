...
// "Jalar datos de contratos" panel — reads contracts from Supabase (via
// the same lightweight snapshot hook Caja uses) and lets the user pick
// a period (semanal or mensual). Calculates ganancia, enCaja, deNuevos,
// deAnteriores for that period and shows clickable cards that auto-fill
// the Caja semanal or Caja mes acumulado inputs in the parent CajaTab.
  ...
  return (
    <div className="mt-4 pt-3 border-t border-zinc-800/60">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-xs text-zinc-500 uppercase font-semibold">📊 Jalar de contratos →</span>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-zinc-500 uppercase font-semibold">Llenar →</span>
          {["semanal", "mensual"].map(t => (
            <button key={t} onClick={() => setTarget(t)} style={btnStyle(target === t)}>
      ...
      {data && data.count === 0 && (
        <div className="text-xs text-zinc-600 mt-2">Sin contratos en este periodo</div>
      )}
    </div>
  )
}
