            ...
            {ct && (
              <div style={{ borderTop:`1px solid ${C.border}`, marginTop:14, paddingTop:14 }}>
                ...
                <div style={{ marginBottom:12 }}>
                  <label style={lbl}>Ubicacion del evento</label>
                  {ct.ubicacion && /^https?:\/\//.test(ct.ubicacion) ? (
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <div style={{ flex:1, padding:"8px 12px", borderRadius:8, background:C.inputBg, border:`1px solid ${C.border}`, fontSize:12, color:C.accent, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", cursor:"pointer" }} onClick={()=>window.open(ct.ubicacion,"_blank")}>
                        {ct.ubicacion}
                      </div>
                      <button onClick={()=>window.open(ct.ubicacion,"_blank")} title="Abrir mapa" style={{ background:C.blue+"22", border:`1px solid ${C.blue}44`, borderRadius:8, color:C.blue, cursor:"pointer", padding:"8px 10px", display:"flex", alignItems:"center" }}>
                        <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>
                      </button>
                      <button onClick={()=>{
                        const msg = `Ubicacion del evento${c.nombre?" de "+c.nombre:""}:\n${ct.ubicacion}`
                        window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, "_blank")
                      }} title="Compartir por WhatsApp" style={{ background:"#25D366"+"22", border:`1px solid #25D36644`, borderRadius:8, color:"#25D366", cursor:"pointer", padding:"8px 10px", display:"flex", alignItems:"center" }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.832-1.438A9.955 9.955 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2z"/></svg>
                      </button>
                      <button onClick={()=>onUpdateContrato(c.id,ct.id,{ubicacion:""})} title="Quitar" style={{ background:C.danger+"22", border:"none", borderRadius:8, color:C.danger, cursor:"pointer", padding:"8px 8px", display:"flex", alignItems:"center", fontSize:12 }}>x</button>
                    </div>
                  ) : (
                    <DInput value={ct.ubicacion||""} onCommit={v=>onUpdateContrato(c.id,ct.id,{ubicacion:v})} style={{ ...inp, marginBottom:0 }} placeholder="Pega link de Google Maps..." />
                  )}
                </div>
