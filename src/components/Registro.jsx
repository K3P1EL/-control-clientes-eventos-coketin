            ...
            {(() => {
              ...
              return (
                <tr key={r.id} onClick={()=>setSelectedRow(isSel?null:r.id)}
                  ...
                  <td style={td}><div style={lock}><TagSelect value={r.estado} onChange={v=>upd(r.id,"estado",v)} tags={tags} getColor={t=>getTagColor(t,tags)} disabled={!canEdit} disabledTags={
                    (r.estado==="Proforma"||r.estado==="Contrato")
                      ? tags.filter(t=>t!=="Proforma"&&t!=="Contrato")
                      : ["Proforma","Contrato"]
                  } /></div></td>
